        ...
        if (msg != null && msg.hasText()) {
            ...
            if (map.get(chatId).isCheckResume()) {
                ...
                if (cmdName.equals("/start")) {
                    ...
                    sendMsg(msg, "Для использования бота вам необходимо узнать ID вашего резюме и токен с dev.hh.ru. "
                            + "Для этого заходите на hh.ru в резюме и копируете из адресной строки набор цифр и букв "
                            + "следующих после \"/resume/\". Затем заходите на dev.hh.ru в личный кабинет, "
                            + "нажимаете запросите токен и копируете его. Также заметьте, что токен выдается на 14 дней, "
                            + "поэтому по завершению срока действия вы получите уведомление о необходимости введения "
                            + "нового токена.");
                }
            }
        }
